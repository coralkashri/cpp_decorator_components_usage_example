...
namespace libraries {
	...
	class json_parser {
	public:
		static components_request_interface_core* get_mission_component(std::type_index &ti, const boost::property_tree::ptree &json) {
			return analyze_mission_component(ti, json);
		}

	private:
		static components_request_interface_core* analyze_mission_component(std::type_index &ti, const boost::property_tree::ptree &json) {
			components_request_missions_basic_mission<> mission_md(json);
			auto mission_name = (components_request_missions_basic_mission_type)mission_md;
			switch (mission_name) {
				case mission_types_parser::MT1:
					ti = typeid(components_request_missions_mt1_mission);
					return new components_request_missions_mt1_mission(json);

				case mission_types_parser::MT2:
					ti = typeid(components_request_missions_mt2_mission);
					return new components_request_missions_mt2_mission(json);
